...
async function runUpstashCommand(command) {
  try {
    ...
    if (err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT') {
      console.warn('[Storage] Upstash timeout, checking connection...');
    }
    throw err;
  }
}
...
async function loadState(defaultState) {
  if (USE_UPSTASH) {
    try {
      ...
      console.warn('[Storage] Upstash read failed, falling back to local file:', err.message);
    }
  }
...
async function saveState(state) {
  if (USE_UPSTASH) {
    try {
      ...
      console.warn('[Storage] Upstash write failed, falling back to local file:', err.message);
    }
  }
  await writeLocalState(state);
}
